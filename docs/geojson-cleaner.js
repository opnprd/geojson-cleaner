var geojsonCleaner = (function (exports) {
  'use strict';
  ...
  var roundToPrecision = function roundToPrecision(value) {
    ...
    var padding = Math.pow(10, precision);
    return Math.round(value * padding) / padding;
  };

  var shorten = function shorten(input) {
  ...
  */
  ...
  function getConverter(crs) {
    if (!Object.prototype.hasOwnProperty.call(projections$1, crs)) {
      throw new Error("Unknown CRS '".concat(crs, "'"));
    }

    return function (c) {
      return proj4(projections$1[crs]).inverse(c);
    };
  }

  function convertFromCrs(data) {
    var name = data.crs.properties.name;
    var converter = getConverter(name);

    var convertCoordinates = function convertCoordinates(coordinate) {
  ...
  function testForCrs(data) {
    var crs = data.crs;

    if (crs) {
      var convertedData = convertFromCrs(data);
  ...
  var osgbPolygon = {
    type: 'Polygon',
    coordinates: [[360000, 550000], [360000, 550000], [360000, 560000], [360000, 550000]]
  };
  var osgbFeatureCollection = {
  ...
  function getInput() {
    var _document$getElementB = document.getElementById('input'),
  ...
  function cleanTextarea() {
    ...
    var result = actions.reduce(function (acc, curr) {
      return curr(acc);
    }, input);
    setResult(result);
  }
